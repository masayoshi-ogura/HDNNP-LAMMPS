...
#include "neighbor.h"
#include "pair_born_effective_charge.h"
...
void PairBORN::get_next_line(ifstream &fin, stringstream &ss, int &nwords) {
  string line;
  int n;
...
void PairBORN::read_file(char *file) {
  ifstream fin;
  stringstream ss;
  ...
  for (i = 0; i < npreprocess; i++) {
    ...
    if (preprocess == "pca") {
      preprocesses.push_back(&PairBORN::pca);
      ...
      preprocesses.push_back(&PairBORN::scaling);
      ...
      preprocesses.push_back(&PairBORN::standardization);
...
void PairBORN::setup_params() {}

/* ---------------------------------------------------------------------- */

void PairBORN::geometry(int i, int *jlist, int jnum, VectorXd *r, VectorXd &R,
...
void PairBORN::feature_index(int *jlist, int jnum, std::vector<int> &iG2s,
                            vector< vector<int> > &iG3s) {
  int i, j, itype, jtype;
  int *type = atom->type;
...
void PairBORN::pca(int type, VectorXd &G, MatrixXd &dG_dx, MatrixXd &dG_dy,
                  MatrixXd &dG_dz) {
...
void PairBORN::scaling(int type, VectorXd &G, MatrixXd &dG_dx, MatrixXd &dG_dy,
                      MatrixXd &dG_dz) {
...
void PairBORN::standardization(int type, VectorXd &G, MatrixXd &dG_dx,
